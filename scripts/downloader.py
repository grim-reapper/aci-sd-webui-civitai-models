...
def dl(url, folder, filename, filepath, model_type):
    ...
    if filepath:
        ...
        if not folder:
            ...
            if 'LORA' == model_type:
                ...
                folder = os.path.join(base_dir, "models/hypernetworks")
            elif 'Controlnet' == model_type:
                folder = os.path.join(base_dir, "models/ControlNet")
            elif 'TextualInversion' == model_type:
                folder = os.path.join(base_dir, "embeddings")
            else:
    ...
    if (total_size < 0):
        ...
        return
    file_size = util.convert_bytes(total_size)
    util.printD(f"File size: {file_size}")
    ...
    # check if downloading file is existed
    downloaded_size = 0
